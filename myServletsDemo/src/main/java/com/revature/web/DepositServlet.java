...
public class DepositServlet extends HttpServlet {
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse res) 
			...
			String enteredDepositAmount = req.getParameter("depositAmount");
			int accNum = Integer.parseInt(enteredAccountNumber);
			double depositAmount = Double.parseDouble(enteredDepositAmount); 
			
			
			System.out.println("Entered Username = " + enteredUsername);
			System.out.println("Entered Password = " + enteredPassword);
			System.out.println("Entered Account Number = " + enteredAccountNumber);
			System.out.println("Entered Deposit Amount = " + enteredDepositAmount);
			...
			if (user != null) {
				...
				if (enteredPassword.equals(user.getPassword())) {
					...
					if (enteredUsername.equals("") || enteredPassword.equals("") || enteredAccountNumber.equals("") || enteredDepositAmount.equals("")) {
			            out.write("<p id='errMsg' style='color: red; font-size: larger;'>Please fill out all fields.</p>");
						rdObj = req.getRequestDispatcher("deposit.html");
						rdObj.include(req, res);
					
					} else {
						String sql = "UPDATE bank_account SET balance = balance + " + depositAmount + " WHERE ACCOUNT_ID = " + enteredAccountNumber + " AND ACCOUNT_OWNER = " + userId;
						...
						try {
							...
							if(result > 0) {
								out.println("<h1>$"+depositAmount+" deposited to "+enteredUsername+ "'s account</h1>");
							} else {
